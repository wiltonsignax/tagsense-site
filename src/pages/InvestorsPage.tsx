...
export function InvestorsPage({locale}:{locale:Locale}) {
  ...
  return <>
    ...
    <FeatureBanner
      eyebrow="PHYSICAL INTELLIGENCE"
      title={locale==='pt'?'Uma tese de software que cresce dentro da operação física.':locale==='en'?'A software thesis that expands inside physical operations.':'Una tesis de software que crece dentro de la operación física.'}
      body={locale==='pt'?'A entrada começa em uma dor mensurável e evolui para múltiplos módulos, dados históricos, integrações e recorrência.':locale==='en'?'Entry starts with a measurable pain point and expands into multiple modules, historical data, integrations and recurring revenue.':'La entrada comienza con un dolor medible y evoluciona hacia múltiples módulos, datos históricos, integraciones y recurrencia.'}
      image="/media/platform_detail_bg.png"
      href={`${pathFor(locale,'contact')}?type=investor`}
      cta={locale==='pt'?'Falar sobre investimento':locale==='en'?'Discuss investment':'Hablar sobre inversión'}
      icon={TrendingUp}
      meta={['SaaS','RFID','Edge','Land & Expand']}
      reverse
    />
  </>
}
